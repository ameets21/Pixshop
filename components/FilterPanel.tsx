...
interface FilterPanelProps {
  ...
  onBatchApply?: (prompt: string, type: 'filter') => void;
  isLoading: boolean;
  isBatchMode?: boolean;
}

const FilterPanel: React.FC<FilterPanelProps> = ({ onApplyFilter, onBatchApply, isLoading, isBatchMode }) => {
  ...
  const handleBatch = () => {
    if (activePrompt && onBatchApply) {
      onBatchApply(activePrompt, 'filter');
    }
  };
    ...
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex flex-col gap-4 animate-fade-in backdrop-blur-sm">
      ...
      {activePrompt && (
        <div className="animate-fade-in flex flex-col sm:flex-row gap-3 pt-2">
            ...
            >
                {isBatchMode ? 'Apply to Current' : 'Apply Filter'}
            </button>
